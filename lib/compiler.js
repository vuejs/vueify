...
var insertCSSPath = JSON.stringify(require.resolve('./insert-css'))
...
compiler.compile = function (content, filePath, cb) {
  ...
  Promise.all(fragment.childNodes.map(function (node) {
    switch (node.nodeName) {
      case 'template':
        return processTemplate(node, filePath, hasScopedStyle)
      case 'style':
        return processStyle(node, filePath)
      case 'script':
  ...
  function mergeParts (parts) {
    ...
    if (style) {
      ...
      output +=
        ...
        'var __vueify_style__ = __vueify_style_insert__.insert(' + style + ');\n'
    }
    // script
      ...
      output +=
        ...
        (style
          ? '  module.hot.dispose(function () {\n' +
            '    __vueify_style_insert__.cache[' + style + '] = false\n' +
...
/**
 * Ensure valid node counts per file.
 *
...
function validateNodeCount (fragment) {
  ...
  fragment.childNodes.forEach(function (node) {
    if (node.nodeName in counts) {
      counts[node.nodeName]
    }
  })
  return counts.template <= 1 && counts.script <= 1
}
...
function processTemplate (node, filePath, hasScopedStyle) {
...
function processStyle (node, filePath) {
