...
compiler.compile = function (content, filePath, cb) {
  ...
  Promise.all([
    ...
    processGraphql(parts.graphql, filePath, resolvedParts),
  ...
  function mergeParts () {
    ...
    var graphqlChanged = resolvedParts.graphql !== prevParts.graphql
    ...
    // graphql
    var graphql = resolvedParts.graphql
    if (graphql) {
      // Put some graphql magic here
    }
    // scoped CSS id
    if (hasScopedStyle) {
...
function processGraphql (part, filePath, parts) {
  if (!part) return Promise.resolve()
  var graphql = getContent(part, filePath)

  return compileAsPromise('graphql', graphql, '', filePath)
    .then(function (res) {
      parts.graphql = res
    })
}
