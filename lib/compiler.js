...
compiler.compile = function (content, filePath, cb) {
  ...
  async.parallel(jobs, function (err) {
    ...
    if (style) {
      ...
      var insertPath = require.resolve('./insert-css').replace(/\\/g, '\\\\')
      output +=
        'var __vueify_style_insert__ = require("' + insertPath + '");\n' +
    ...
    if (template) {
      ...
      output += 'var __vueify_template__ = ' + JSON.stringify(template) + ';\n'
    }
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      output +=
        'if (module.hot) {(function () {' +
          ...
          'var id = module.exports.hotID = "' + filePath + '"\n' +
